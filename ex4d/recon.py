...
DEPTHCRAFTER_PATH = PROJECT_ROOT / "DepthCrafter"
if str(DEPTHCRAFTER_PATH) not in sys.path:
    sys.path.append(str(DEPTHCRAFTER_PATH))

from ex4d.utils.nv_utils import *
from ex4d.utils.dc_utils import read_images_frames, read_video_frames
from ex4d.utils.render_utils import get_rays_from_pose

from ex4d.utils.depth_utils import DepthCrafterDemo
...
def random_camera_traj(n_frames, depth_src, random_type, args, depth_min=None, compare=True, fixed_frame=49, random_iter=0):
    ...
    if random_type == "180":
        ...
        centers[:, 2] = radius
    elif random_type == "zoom_in":
        radius = depth_min
        eyes = np.zeros((args.num_frames, 3))
        angle = np.linspace(0, radius - 0.01, args.num_frames)
        eyes[:, 2] = angle

        centers = np.zeros((args.num_frames, 3))
        centers[:, 2] = radius
    elif random_type == "zoom_out":
        radius = depth_min
        eyes = np.zeros((args.num_frames, 3))
        angle = np.linspace(0, radius, args.num_frames)
        eyes[:, 2] = -angle
...
def run_render(video_path, output_dir, args, models):
    ...
    frames = np.array([cv2.resize(frame, (384, 384)) for frame in frames])
    ...
    with torch.no_grad():
        ...
        for idx, poses in enumerate(tqdm(camera_poses)):
            ...
            vertices, new_faces, colors, _ = point_to_mesh_cuda(pts_xyz, pts_color, faces)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Recon Example')
    # parser.add_argument('--input_video', type=str, default="examples/flower/input.mp4")
    parser.add_argument('--input_video', type=str, default="openvid_part100/extracted/mnt/bn/videodataset-uswest/VDiT/dataset/panda-ours/_dcnQgJj-qg_4_0to207.mp4")
    ...
    # parser.add_argument('--name', type=str, default="cat")
