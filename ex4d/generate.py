...
# Ensure EX-4D scripts under ex4d/ can still access project-root shared modules.
...
def run_inference(args, pipe, color_video_path, mask_video_path, output_video_path):
    ...
    if args.skip_existing and os.path.exists(output_video_path):
        print(f"Skip existing output: {output_video_path}")
        return
    ...
    mask_tensor = load_mask_frames(mask_video_path, args.num_frames, args.height, args.width)
    ...
    save_video(output_video, output_video_path, fps=15, quality=8)
    print("Done!")


def main():
    parser = argparse.ArgumentParser(description='Video Inference with EX-4D')
    parser.add_argument('--color_video', type=str, help='Path to condition video')
    parser.add_argument('--mask_video', type=str, help='Path to mask video')
    ...
    parser.add_argument('--batch_list', type=str, help='Path to batch list file (color|mask|output per line)')
    ...
    if args.batch_list:
        if not os.path.exists(args.batch_list):
            raise FileNotFoundError(f"Batch list not found: {args.batch_list}")
        with open(args.batch_list, "r", encoding="utf-8") as handle:
        ...
        for line in lines:
            parts = line.split("|")
            if len(parts) != 3:
                raise ValueError(f"Invalid batch line (expected 3 fields): {line}")
            color_video_path, mask_video_path, output_video_path = parts
            run_inference(args, pipe, color_video_path, mask_video_path, output_video_path)
    else:
        if not args.color_video:
            raise ValueError("--color_video is required when --batch_list is not provided")
        if not args.mask_video or not args.output_video:
            raise ValueError("--mask_video and --output_video are required when --batch_list is not provided")
        pipe = load_pipeline(args)
        run_inference(args, pipe, args.color_video, args.mask_video, args.output_video)
